...
if __name__ == "__main__":
    ...
    auth = {'username': config.get(
        'auth', 'username'), 'password': config.get('auth', 'password')}
    ...
    properties = [(key, value) for key, value in (prop.split(',')
                                                  for prop in propertiesRaw.split(';') if prop.find(',') > -1)]
